...
import { Mail, Lock, Eye, EyeOff, Loader2, Leaf, AlertCircle } from 'lucide-react';
    ...
    try {
      ...
      try {
        ...
        if (textResponse === 'NOT_FOUND') {
          throw new Error("Endpoint API tidak ditemukan (404 NOT_FOUND). Pastikan rute POST '/api/v1/company/auth' sudah dibuat di backend Elysia kamu.");
        } else {
      ...
      if (response.ok && (result.status === 'success' || result.token)) {
        ...
        setErrorMessage(result.message || 'Login gagal. Periksa kembali email dan password Anda.');
      }
      
    } catch (error: any) {
      console.error('Login error:', error);
      setErrorMessage(error.message || 'Terjadi kesalahan jaringan atau server tidak merespons.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center bg-[#FAFAFA] overflow-hidden font-sans">
      ...
      <div className="absolute bottom-0 left-0 w-full h-full pointer-events-none z-0 flex items-end">
        ...
        >
          <path d="M0,160L48,170.7C96,181,192,203,288,213.3C384,224,480,224,576,202.7C672,181,768,139,864,128C960,117,1056,139,1152,160C1248,181,1344,203,1392,213.3L1440,224L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" fillOpacity="0.7"></path>
          <path d="M0,256L60,245.3C120,235,240,213,360,208C480,203,600,213,720,234.7C840,256,960,288,1080,288C1200,288,1320,256,1380,240L1440,224L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z" fillOpacity="1"></path>
        </svg>
