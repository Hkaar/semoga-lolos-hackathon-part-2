...
const BANK_OPTIONS = [
  { id: 'bca', name: 'BCA', acc: '8291 0029 11', owner: 'PT Klimabot Teknologi Hijau' },
  { id: 'mandiri', name: 'Mandiri', acc: '137 000 291 8821', owner: 'PT Klimabot Teknologi Hijau' },
  { id: 'bri', name: 'BRI', acc: '0219 0100 2911 301', owner: 'PT Klimabot Teknologi Hijau' }
];
...
export default function ProfilePage() {
  ...
  const [subSelectedBank, setSubSelectedBank] = useState<typeof BANK_OPTIONS[0] | null>(null);
  
  const [isProcessing, setIsProcessing] = useState(false);
  const [toastMessage, setToastMessage] = useState<{show: boolean, msg: string, isPending?: boolean}>({ show: false, msg: '' });
  ...
  const showToast = (msg: string, isPending: boolean = false) => {
    ...
    setTimeout(() => setToastMessage({ show: false, msg: '', isPending: false }), 4000);
  };
  ...
  const handleConfirmTopUp = async () => {
    ...
    showToast(`Instruksi Top Up ${formatRupiah(Number(topUpAmount))} dikirim. Menunggu verifikasi admin.`, true);
  };
  ...
  const handleConfirmSubscribe = async () => {
    ...
    showToast(`Konfirmasi perpanjangan diterima. Menunggu verifikasi admin.`, true);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] p-4 md:p-8 font-sans pb-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">PT Indofood Sukses Makmur Tbk.</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          >
            <div className="space-y-4">
                  ...
                  {FOCUS_CATEGORIES.map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <p className={`text-sm font-medium ${active ? "text-green-800" : "text-gray-600"}`}>
                            {cat.label}
                          </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs font-medium text-gray-500 mb-1.5">Target Wilayah CSR</p>
                  <div className="relative">
                    <button
                      onClick={() => setShowWilayahDropdown(!showWilayahDropdown)}
                        ...
                        {WILAYAH_OPTIONS.map((opt) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-green-50 transition-colors ${
                              wilayah === opt ? "text-green-700 font-medium bg-green-50/50" : "text-gray-600"
                            }`}
                          >
                            {opt}
                          </button>
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500 mb-1.5">Target Volume Tahunan</p>
              ...
              <p className="text-xs text-gray-400 flex items-center gap-1.5">
                <Info size={12} /> Target ini akan digunakan untuk menghitung progress dampak di dashboard.
              </p>
            </div>
          </SectionCard>
          ...
          >
            <div className="divide-y divide-gray-50">
              {[
                { label: "Nama Perusahaan", value: "PT Indofood Sukses Makmur Tbk." },
                { label: "Nomor Induk Berusaha (NIB)", value: "8120391xxxxxx" },
                { label: "Sektor Industri", value: "FMCG (Fast Moving Consumer Goods)" },
                { label: "Alamat Kantor Pusat", value: "Sudirman Plaza,\nJakarta Selatan 12920" },
              ].map((row) => (
                <div key={row.label} className="flex gap-3 py-3">
                  <p className="text-sm text-gray-400 w-44 flex-shrink-0">{row.label}</p>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5 leading-tight">Akses penuh ke semua fitur premium</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              ...
              <p className="text-sm font-bold text-gray-900">
                Rp 15.000.000 <span className="font-normal text-gray-400 text-xs">/ Bulan</span>
              </p>
              <p className="text-xs text-gray-400 mt-0.5 leading-tight">Termasuk semua fitur & dukungan prioritas</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-green-500 inline-block" /> Aktif
                </p>
                <p className="text-xs text-gray-400 mt-0.5">Next billing: {formatDate(billingDate)}</p>
              </div>
            ...
            <div className="bg-green-50/40 rounded-xl p-4 border border-green-100 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600 mb-0.5">Sisa Saldo Dana CSR</p>
                <p className="text-lg font-bold text-gray-900">{formatRupiah(saldoCSR)}</p>
                <p className="text-xs text-green-700/60 mt-0.5">Alokasi insentif untuk warga</p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              {topUpStep === 1 ? (
                <>
                  <p className="text-sm text-gray-500 mb-4">Pilih nominal instan atau masukkan jumlah kustom (Minimum Rp 1.000.000).</p>
                  ...
                  <p className="text-sm font-semibold text-gray-800 mb-3">Metode Transfer Bank</p>
                    ...
                    {BANK_OPTIONS.map((bank) => (
                      <div 
                        ...
                        className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-all ${
                          selectedBank?.id === bank.id ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-green-300'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <Landmark className={`w-5 h-5 ${selectedBank?.id === bank.id ? 'text-green-600' : 'text-gray-400'}`} />
                          <span className="text-sm font-semibold text-gray-700">Bank {bank.name}</span>
                        </div>
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                          selectedBank?.id === bank.id ? 'border-green-600' : 'border-gray-300'
                        }`}>
                          {selectedBank?.id === bank.id && <div className="w-2 h-2 rounded-full bg-green-600" />}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button 
                      ...
                      disabled={!topUpAmount || topUpAmount < 1000000 || !selectedBank}
                ...
                <div className="animate-in slide-in-from-right-4 duration-300">
                  <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-3 rounded-xl text-xs flex items-start gap-2 mb-4">
                    ...
                    <span>Selesaikan pembayaran dalam <b>24 jam</b> ke depan agar dana bisa segera masuk ke saldo Anda.</span>
                  </div>

                  <p className="text-xs text-gray-500 mb-1">Transfer tepat sebesar:</p>
                  <p className="text-2xl font-bold text-gray-900 mb-6">{formatRupiah(Number(topUpAmount))}</p>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 relative">
                    ...
                    <p className="text-sm font-bold text-gray-900 mb-3">Bank {selectedBank?.name}</p>
                    
                    <p className="text-xs text-gray-500 mb-1">Nomor Rekening</p>
                    <div className="flex items-center gap-2 mb-3">
                      <p className="text-lg font-mono font-bold tracking-wider text-green-700">{selectedBank?.acc}</p>
                      <button onClick={() => copyToClipboard(selectedBank?.acc || '')} className="p-1.5 bg-green-100 text-green-700 rounded-md hover:bg-green-200">
                        <Copy className="w-4 h-4" />
                      </button>
                    </div>

                    <p className="text-xs text-gray-500 mb-1">Atas Nama</p>
                    <p className="text-sm font-bold text-gray-900">{selectedBank?.owner}</p>
                  </div>
                  ...
                  >
                    {isProcessing ? <Loader2 size={16} className="animate-spin" /> : 'Saya Sudah Transfer'}
                  </button>
                  <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              ...
              <button onClick={() => setIsSubscribeModalOpen(false)} className="text-gray-400 hover:text-gray-600">
                <X size={20} />
              </button>
            </div>
                ...
                <>
                  <div className="bg-green-50/50 border border-green-100 rounded-xl p-4 mb-6">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-semibold text-gray-900">Enterprise Plan (SaaS)</span>
                      <span className="text-xs px-2 py-1 bg-green-100 text-green-700 font-bold rounded-full">1 Bulan</span>
                    </div>
                    <p className="text-xs text-gray-500 mb-3">Masa aktif langganan Anda akan diperpanjang hingga <strong className="text-gray-700">{
                      formatDate(new Date(new Date(billingDate).setMonth(billingDate.getMonth() + 1)))
                    }</strong>.</p>
                    ...
                    <div className="pt-3 border-t border-green-100 flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold text-green-700">Rp 15.000.000</span>
                    </div>
                  </div>

                  <p className="text-sm font-semibold text-gray-800 mb-3">Pilih Metode Pembayaran</p>
                  <div className="space-y-2 mb-6">
                    {BANK_OPTIONS.map((bank) => (
                      <div 
                        key={bank.id}
                        ...
                        className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-all ${
                          subSelectedBank?.id === bank.id ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-green-300'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <Landmark className={`w-5 h-5 ${subSelectedBank?.id === bank.id ? 'text-green-600' : 'text-gray-400'}`} />
                          <span className="text-sm font-semibold text-gray-700">Bank {bank.name}</span>
                        </div>
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                          subSelectedBank?.id === bank.id ? 'border-green-600' : 'border-gray-300'
                        }`}>
                          {subSelectedBank?.id === bank.id && <div className="w-2 h-2 rounded-full bg-green-600" />}
                        </div>
                      </div>
                    ))}
                  </div>
                 ...
                 <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-3 rounded-xl text-xs flex items-start gap-2 mb-4">
                   <Clock className="w-4 h-4 flex-shrink-0 mt-0.5" />
                   <span>Selesaikan pembayaran dalam <b>24 jam</b> agar langganan segera diperpanjang.</span>
                 </div>

                 <p className="text-xs text-gray-500 mb-1">Transfer tepat sebesar:</p>
                 <p className="text-2xl font-bold text-gray-900 mb-6">Rp 15.000.000</p>

                 <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 relative">
                   <p className="text-xs text-gray-500 mb-1">Bank Tujuan</p>
                   <p className="text-sm font-bold text-gray-900 mb-3">Bank {subSelectedBank?.name}</p>
                   
                   <p className="text-xs text-gray-500 mb-1">Nomor Rekening</p>
                   <div className="flex items-center gap-2 mb-3">
                     <p className="text-lg font-mono font-bold tracking-wider text-green-700">{subSelectedBank?.acc}</p>
                     <button onClick={() => copyToClipboard(subSelectedBank?.acc || '')} className="p-1.5 bg-green-100 text-green-700 rounded-md hover:bg-green-200">
                       <Copy className="w-4 h-4" />
                     </button>
                   </div>

                   <p className="text-xs text-gray-500 mb-1">Atas Nama</p>
                   <p className="text-sm font-bold text-gray-900">{subSelectedBank?.owner}</p>
                 </div>
      ...
      {/* --- TOAST NOTIFICATION --- */}
      {toastMessage.show && (
        <div className={`fixed bottom-6 right-6 z-50 ${toastMessage.isPending ? 'bg-yellow-50 border-2 border-yellow-200 text-yellow-800' : 'bg-gray-900 text-white'} px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 animate-in slide-in-from-bottom-5 fade-in duration-300`}>
          {toastMessage.isPending ? (
            <Clock className="w-5 h-5 text-yellow-600" />
          ) : (
            <CheckCircle2 className="w-5 h-5 text-green-400" />
          )}
          <p className="text-sm font-medium max-w-[280px]">{toastMessage.msg}</p>
        </div>
      )}
